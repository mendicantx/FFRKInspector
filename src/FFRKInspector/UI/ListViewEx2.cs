...
// Type: FFRKInspector.UI.ListViewEx2
...
namespace FFRKInspector.UI
{
  public class ListViewEx2 : ListView
  {
    ...
    private List<ListViewEx2.ListViewFieldAdapter> mFields;
    ...
    public ListViewEx2()
    {
      ...
      this.mFields = new List<ListViewEx2.ListViewFieldAdapter>();
      this.InitializeComponent();
      this.InitializeComponent2();
      this.HandleCreated += new EventHandler(this.ListViewEx2_HandleCreated);
      this.RetrieveVirtualItem += new RetrieveVirtualItemEventHandler(this.ListViewEx2_RetrieveVirtualItem);
      this.ColumnClick += new ColumnClickEventHandler(this.ListViewEx2_ColumnClick);
      this.ColumnWidthChanged += new ColumnWidthChangedEventHandler(this.ListViewEx2_ColumnWidthChanged);
    ...
    private static extern int EnumChildWindows(
      IntPtr HwndParent,
      ListViewEx2.EnumWinCallback Callback,
      IntPtr LParam);

    [DllImport("user32.dll")]
    private static extern bool GetWindowRect(IntPtr Hwnd, out ListViewEx2.RECT Rect);
    ...
    {
      ListViewEx2.ListViewFieldAdapter viewFieldAdapter = new ListViewEx2.ListViewFieldAdapter()
      {
        Field = Field,
    ...
    private void ListViewEx2_ColumnWidthChanged(object sender, ColumnWidthChangedEventArgs e)
    {
    ...
    private void ListViewEx2_ColumnClick(object sender, ColumnClickEventArgs e)
    {
      ...
      ListViewEx2.ListViewFieldAdapter Field = this.mFields.Find((Predicate<ListViewEx2.ListViewFieldAdapter>) (x => x.Column == clicked_column));
    ...
    private void ListViewEx2_HandleCreated(object sender, EventArgs e)
    {
      this.mHeaderContextMenuStrip = new ContextMenuStrip();
      foreach (ColumnHeader columnHeader in this.mFields.Select<ListViewEx2.ListViewFieldAdapter, ColumnHeader>((Func<ListViewEx2.ListViewFieldAdapter, ColumnHeader>) (x => x.Column)))
      {
        ...
        toolStripMenuItem.Tag = (object) new ListViewEx2.ToolStripItemTag()
        {
          Header = columnHeader,
    ...
    private void ListViewEx2_RetrieveVirtualItem(object sender, RetrieveVirtualItemEventArgs e)
    {
      ...
      foreach (ListViewEx2.ListViewFieldAdapter mField in this.mFields)
      {
    ...
    private void item_CheckStateChanged(object sender, EventArgs e)
    {
      ...
      ListViewEx2.ToolStripItemTag tag = (ListViewEx2.ToolStripItemTag) toolStripMenuItem.Tag;
      ColumnHeader column_to_change = tag.Header;
      this.mFields.Find((Predicate<ListViewEx2.ListViewFieldAdapter>) (x => x.Column == column_to_change));
    ...
    private int DetermineIndexForInsertingColumn(ColumnHeader Header) => this.mFields.TakeWhile<ListViewEx2.ListViewFieldAdapter>((Func<ListViewEx2.ListViewFieldAdapter, bool>) (x => x.Column != Header)).Count<ListViewEx2.ListViewFieldAdapter>((Func<ListViewEx2.ListViewFieldAdapter, bool>) (x => x.Column.DisplayIndex != -1));

    private bool EnumWindowCallback(IntPtr Hwnd, IntPtr LParam)
    {
      ListViewEx2.RECT Rect;
      this.mHeaderRect = ListViewEx2.GetWindowRect(Hwnd, out Rect) ? new Rectangle(Rect.Left, Rect.Top, Rect.Right - Rect.Left + 1, Rect.Bottom - Rect.Top + 1) : Rectangle.Empty;
      return false;
    }
      ...
      try
      {
        ...
        if (saveFileDialog.ShowDialog() == DialogResult.OK)
        {
          using (Stream stream = saveFileDialog.OpenFile())
          {
            ...
            {
              List<ListViewEx2.ListViewFieldAdapter> list = this.mFields.Where<ListViewEx2.ListViewFieldAdapter>((Func<ListViewEx2.ListViewFieldAdapter, bool>) (x => x.Column.DisplayIndex != -1)).ToList<ListViewEx2.ListViewFieldAdapter>();
              ...
              foreach (int row in Rows)
              {
                ...
                for (int index = 0; index < list.Count; ++index)
                {
                  ListViewEx2.ListViewFieldAdapter viewFieldAdapter = list[index];
    ...
    {
      ListViewEx2.EnumChildWindows(this.Handle, new ListViewEx2.EnumWinCallback(this.EnumWindowCallback), IntPtr.Zero);
